...
Твёрдо установлено три факта. **Корректор физику не использует**: косинус между его
поправкой и δ_phys равен **−0.002**, то есть они ортогональны. Это механистическое
подтверждение того, что до сих пор было видно только по RMSE (R3a ≈ R0). **География
бьёт ядро в 6–20 раз**: она объясняет 20–26 % дисперсии выхода Q_θ, поля ядра
(`cond`, ω) — всего 1–3 %. **Q_θ — не нетто-источник** (среднее −0.068 по влаге), что
независимо подтверждает отсутствие сдвига bias и **хоронит наивную версию замыкания**
(«Q_θ — это недостающее испарение»).
