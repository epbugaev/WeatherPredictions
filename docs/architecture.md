...
### 3.0. Ключ ко всему: где в этой схеме НОВАЯ информация

Прежде чем разбирать механику, стоит посмотреть на потоки данных глазами теории
информации — это объясняет результаты лестницы лучше, чем любые рассуждения про
уравнения.

**δ_phys не несёт ни бита новой информации.** Он равен `physics(prev) − prev`, то есть
**детерминированной функции от `prev`** — а `prev` корректор получает на вход и так.
Ядро в режиме `stable_physical_v2` не имеет **ни одного обучаемого параметра**
(`physics_only_forward` обходит все свёртки и BN). Значит, физический приор — это
вручную посчитанное нелинейное преобразование того, что у сети уже есть. Он может быть
полезен **только как индуктивное смещение** — избавить сеть от труда посчитать это
самой. Добавить знания он не может **в принципе**.

Отсюда сразу читается главный результат лестницы: R3a − R0 = +0.22 % (CI накрывает
ноль) означает, что это избавление от труда **не стоит ничего**.

**А вот география — единственная новая информация во всей модели.** В данных четыре
приземных канала: `t2`, `u10`, `v10`, `tp`. Ни орографии, ни маски суша/море, ни поля
широты там нет — **backbone географии не видит вовсе**. Единственный модуль, который её
видит, — Q_θ (`[prev, гео]`).

Тогда всё измеренное складывается в одну картину:
...
| Q_θ не нетто-источник, bias не двигает | это структурная поправка, привязанная к рельефу, а не источник |
...
**Рабочая гипотеза, которую проверяют армы X1/X2 (§7): весь «физический» выигрыш
PI-IAM4VP — это статический гео-эмбеддинг, а примитивные уравнения — сложный способ
протащить в модель три постоянные карты.**
...
**РЕЗУЛЬТАТ (прогнано, 2026-07-14; полностью —
[closure/README.md](experiments/16_model_ablation_ladder/closure/README.md)).**
Предпроверка склоняется к «свободному источнику», но вопрос не закрывает — и по пути
находит вещь, которой в плане не было.

**Корректор физику ИСПОЛЬЗУЕТ.** Косинус между поправкой и δ_phys равен −0.002, и
первое прочтение — «признак в модель не входит» — было **ошибочным и отозвано**:
ортогональность значит лишь, что поправка не *сонаправлена* с δ_phys. Прямая абляция на
обученном R3 (веса не тронуты) это опровергает: занулить δ_phys на инференсе — **+0.50 %**
RMSE, подменить его тенденцией **чужого дня** — **+20.8 %**. Сеть на признак опирается,
и сильно.

**Но «используется» ≠ «полезен»** — и разгадка ровно в §3.0. δ_phys информационно
избыточен, поэтому сеть охотно берёт даровую предвычисленную нелинейность (отнять
больно), отравляется, если её подменить ложью (катастрофа), — но, обученная **без** неё
с нуля, выучивает эквивалент сама и не проигрывает ничего (R3a − R0 = +0.22 %). Это
поведение калькулятора, а не знания.

**География бьёт ядро в 6–20 раз**: она объясняет 20–26 % дисперсии выхода Q_θ, поля
ядра (`cond`, ω) — всего 1–3 %. **Q_θ — не нетто-источник** (среднее −0.068 по влаге),
что независимо подтверждает отсутствие сдвига bias и **хоронит наивную версию
замыкания** («Q_θ — это недостающее испарение»).

Но есть и встречный факт: **Q_θ меняет знак в режиме конденсации** — вне зоны
насыщенного подъёма он сушит (−0.084), внутри увлажняет (**+0.363**). Выглядит ровно
как ремонт стока. **Однако это ничего не доказывает**: условие насыщения q ≥ q_s —
локальная поточечная функция от (T, q, p), и свёртка выучивает её тривиально, безо
всякого ядра. Единственная часть режима, которую локальный CNN взять **не может**, —
это ω < 0 (нелокальный вертикальный интеграл дивергенции), и корреляция именно с ω
оказалась **слабейшей** (−0.12).

Уточнённое предсказание: **X1 ближе к −3 %, чем к нулю**; взаимодействие мало;
уравнения, скорее всего, декорация. И **X2 из желательного стал обязательным**: 20–26 %
географии внутри Q_θ означают, что без него нельзя утверждать, что «диабатика» — это
диабатика, а не эмбеддинг рельефа.
