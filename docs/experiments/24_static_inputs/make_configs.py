...
# Какие (семейство, арм, тег-набора) генерировать.
#   Полный static (orog+lsm) — для всех 3 семейств × 2 арма (исходная матрица 6).
#   orography-only — только IAM4VP × 2 арма (чистый A/B «без/с орографией»).
JOBS: list[tuple[str, str, str]] = [
    *[(fam, arm, "static") for (fam, arm) in BASE_CONFIGS],
    ("iam4vp", "nophys", "orog"),
    ("iam4vp", "a2", "orog"),
]


def staticize(config: dict, family: str, arm: str, fields: list[str], tag: str) -> dict:
    ...
    config["experiment"]["name"] = f"exp24-{family}-{arm}-{tag}-usa-s0"
...
def main() -> None:
    ...
    for family, arm, tag in JOBS:
        ...
        config = staticize(yaml.safe_load(base_path.read_text()), family, arm, fields, tag)
        out_path = OUT_DIR / f"exp24_{family}_{arm}_{tag}_usa.yaml"
        header = (
            f"# GENERATED by docs/experiments/24_static_inputs/make_configs.py "
            f"from {base_rel}.\n# Do not edit by hand — regenerate. "
            f"Arm={arm}, family={family}, static={'+'.join(fields)}, region=usa.\n"
        )
