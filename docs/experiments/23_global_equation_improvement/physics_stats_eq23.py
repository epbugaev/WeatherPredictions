...
from utils.physics import (  # noqa: E402
    Grid,
    GridConfig,
    PhysicsConstants,
    PurePDEKernel,
    latent_heating_profile,
)
from utils.physics_hybrid import relative_to_specific_humidity  # noqa: E402

# Эксперимент 23: наблюдаемые поля форсинга. Нативную q читаем напрямую
# (убирает ошибку реконструкции Magnus); осадки → латентное тепло T.
NATIVE_Q = os.environ.get("NATIVE_Q", "1") == "1"  # нативная specific_humidity
USE_PRECIP = os.environ.get("USE_PRECIP", "1") == "1"  # источник латентного тепла
...
# --- Эксперимент 23: новые члены наблюдаемого форсинга (opt-in) ---
# База exp23 = принятый предел exp15 (C15). Каждый член добавляется поверх.
E23B_KW = dict(C15_KW, virtual_temperature=True)  # виртуальная температура
E23D_KW = dict(C15_KW, bulk_drag=True)  # bulk-трение пограничного слоя
# E23-C (латентное тепло осадков) реализован как внешний source (PRECIP_VARIANTS),
# не как флаг ядра. E23-A (нативная q) — глобальный вход (NATIVE_Q), влияет
# на все варианты. Комбинация: виртуальная T + bulk-трение (+ осадки в C23_precip).
C23_KW = dict(C15_KW, virtual_temperature=True, bulk_drag=True)
...
VARIANTS: dict[str, tuple[str, dict]] = {
    ...
    "C15_now": ("exact", C15_KW),  # предел exp15 = база exp23
    # --- кандидаты exp23 (каждый = C15 + один член) ---
    "E23B_vtemp": ("exact", E23B_KW),  # виртуальная температура в гидростатике
    "E23D_drag": ("exact", E23D_KW),  # bulk-трение пограничного слоя
    "C23_vtdrag": ("exact", C23_KW),  # виртуальная T + bulk-трение
}
# exp23 отключает климатологические источники exp15 (Q₁/Q₂ клим.) — их
# заменяет НАБЛЮДАЕМЫЙ форсинг осадков (PRECIP_VARIANTS), считаемый на лету.
SOURCE_VARIANTS: dict[str, tuple[str, str]] = {}
# E23-C: базовое ядро + наблюдаемое латентное тепло осадков (source T).
# name → базовый вариант; источник Q добавляется в t_t на каждом шаге.
PRECIP_VARIANTS = {
    "E23C_precip": "C15_now",  # C15 + латентное тепло осадков
    "C23_final": "C23_vtdrag",  # виртуальная T + bulk-трение + осадки (полная)
}
FOCUS = ("base13", "C_best", "C15_now")
MAP_SET = ("base13", "C15_now", "C23_vtdrag", "C23_final")
CLIM_REFS = ("C_best", "C15_now")  # против каких ядер копится климатология (не исп.)
...
class Era5Reader:
    ...
    def __init__(self, dom: dict, grid: Grid) -> None:
        ...
        # exp23: нативная удельная влажность (specific_humidity/q) — 13 уровней.
        self.q_file = None
        if NATIVE_Q:
            self.q_file = h5netcdf.File(
                f"{ERA5_ROOT}specific_humidity/specific_humidity_{YEAR}_1.40625deg.nc", "r"
            )
        # exp23: суммарные осадки (total_precipitation/tp) — приземное поле,
        # м водного эквивалента за час → скорость P [м/с] = tp / 3600.
        self.tp_file = None
        if USE_PRECIP:
            self.tp_file = h5netcdf.File(
                f"{ERA5_ROOT}total_precipitation/total_precipitation_{YEAR}_1.40625deg.nc", "r"
            )
        self.precip_cache: dict[int, torch.Tensor] = {}
    ...
    def state(self, hour: int) -> dict[str, torch.Tensor]:
        ...
        if self.q_file is not None:
            # exp23 E23-A: нативная specific_humidity вместо инверсии Magnus.
            q_arr = np.asarray(
                self.q_file.variables["q"][hour, 0:13, self.dom["rows"], self.dom["cols"]],
                dtype=np.float32,
            )
            q = torch.from_numpy(q_arr)[None]
        else:
    ...
    def precip_rate(self, hour: int) -> torch.Tensor | None:
        """Скорость осадков P [м/с в.э.] формы (1,1,H,W); None если выключено.

        ERA5 total_precipitation — накопленные осадки [м в.э.] за час; делим
        на 3600 с. Отрицательный шум накопления обрезается нулём.
        """
        if self.tp_file is None:
            return None
        if hour in self.precip_cache:
            return self.precip_cache[hour]
        tp = np.asarray(
            self.tp_file.variables["tp"][hour, self.dom["rows"], self.dom["cols"]],
            dtype=np.float32,
        )
        rate = torch.from_numpy(tp)[None, None].clamp_min(0.0) / 3600.0
        self.precip_cache[hour] = rate
        for old in [h for h in self.precip_cache if h < hour - 2]:
            del self.precip_cache[old]
        return rate
...
def run() -> dict:
    ...
    active_precip_variants = dict(PRECIP_VARIANTS) if USE_PRECIP and not SYNTHETIC else {}
    all_names = list(VARIANTS) + list(active_source_variants) + list(active_precip_variants)
    # вертикальный профиль латентного тепла η(σ)=sin(πσ), общий для всех точек.
    precip_pressure_hpa = exact_grid.pressure.reshape(-1) / 100.0
    ...
    for i, h in enumerate(triple_starts):
        ...
        with torch.no_grad():
            ...
            # exp23 E23-C: наблюдаемое латентное тепло осадков как источник T.
            # Q = latent_heating_profile(P) добавляется к t_t базового варианта.
            if active_precip_variants:
                precip = reader.precip_rate(h)  # (1,1,H,W) м/с в.э.
                q_lh = latent_heating_profile(
                    precip, exact_grid.pixel_z, consts, pressure_hpa=precip_pressure_hpa
                )
                for sname, ref in active_precip_variants.items():
                    model_t[sname] = {
                        v: model_t[ref][v] + q_lh if v == "t" else model_t[ref][v]
                        for v in ALL_KEYS
                    }
    ...
    return {
        "meta": {
            ...
            "precip_variants": active_precip_variants,
            "native_q": NATIVE_Q,
