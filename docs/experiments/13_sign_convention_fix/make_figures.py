...
def fig1_residual_stages() -> None:
    ...
    add_caption(
        fig,
        "Число — во сколько раз физика ошибается: её ошибка, делённая на то, как поле реально\n"
        "изменилось за час. Единица — это уже плохо (промах на всю величину изменения), сто —\n"
        "совсем плохо; чем меньше число и чем левее точка, тем лучше. Светлые точки справа —\n"
        "старое ядро с перепутанными знаками, тёмные слева — после нашего исправления.\n"
        "Тёмные стоят левее светлых в 10–30 раз: исправление сильно приблизило физику к погоде.",
        y=-0.52,
    )
...
def fig2_omega_maps() -> None:
    ...
    add_caption(
        fig,
        "Синее — воздух поднимается, красное — опускается, белое — стоит на месте; чёрные линии —\n"
        "берега. Слева — вертикальная скорость, посчитанная прямо из ветра (так делает код); справа —\n"
        "та, что на самом деле нужна, чтобы объяснить нагрев воздуха. Было бы хорошо, если бы они\n"
        "совпадали — но левая это сплошная яркая мешанина (шум), а правая спокойная и с другим\n"
        "рисунком. Значит скорость, взятую напрямую из ветра, использовать нельзя. В центре —\n"
        "промежуточный вариант: шумит слабее, но с нужной картинкой всё равно не совпадает.",
    )
...
def fig3_usa_maps() -> None:
    ...
    add_caption(
        fig,
        "Четыре карты окна над США; чёрная линия — берега.\n"
        "Слева вверху — просто погода в этот день, для ориентира: темнее оранжевый — теплее, серые линии — рельеф давления.\n"
        "Справа вверху — ошибка температуры: белое хорошо (физика угадала), красное и синее плохо (недооценила или переоценила\n"
        "нагрев). Пятен ещё много — физика неидеальна, но грубых промахов уже нет.\n"
        "Слева внизу — где идёт конденсация: синие пятна значат, что она наконец работает; до исправления это поле было пустым\n"
        "(физика вообще не давала осадков). Справа внизу — куда движется воздух: синее вверх, красное вниз.",
    )
...
def fig4_lat_profiles() -> None:
    ...
    add_caption(
        fig,
        "Чем ниже линия, тем лучше физика описывает поле. По горизонтали — широта: центр это экватор, края — полюса.\n"
        "Видно две вещи. Все линии загибаются вверх к краям — у полюсов физика ошибается сильнее, чем над серединой.\n"
        "И оранжевая линия (ветер с юга на север) всюду выше синей (ветер с запада на восток) — именно этот ветер\n"
        "физике даётся хуже всего. Температура (зелёная) и геопотенциал (розовая) идут ниже — с ними после исправления порядок.",
    )
...
def fig5_cfl_map() -> None:
    ...
    add_caption(
        fig,
        "Число Куранта показывает, поспевает ли расчёт за ветром: пока меньше 1 — счёт устойчив, как только больше 1 —\n"
        "идёт вразнос. Светло-голубое хорошо: всё спокойно, с большим запасом; чем темнее, тем ближе к опасной черте;\n"
        "красная линия — это и есть черта «1», чёрные линии — берега. Почти весь мир и окно над США светлые — счёт\n"
        "устойчив. Красное появляется только у самой кромки карты (у полюсов), где ячейки сетки вырождаются, — там,\n"
        "и только там, шаг 300 с слишком велик. Наибольшее число по 96 снимкам: у полюсов "
        f"{band_max['polar']:.1f}, в средних широтах {band_max['midlat']:.2f},\n"
        f"в тропиках {band_max['tropics']:.2f}, над США — {RESULTS['usa']['cfl_dt300']['adv_max']:.3f}.",
    )
