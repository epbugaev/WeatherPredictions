...
## 11. Результаты (промежуточные, 4/6 лестниц)

Готовы SimVPv2 и IAM4VP на обоих регионах (все армы на общей эпохе 150). PredRNNv2
(бюджет 40, §2) досчитывается — france вероятно успеет к дедлайну аллокации, npac
под вопросом; будет добавлен по готовности. Δ к `no_physics`, среднее по upper-каналам,
парный бутстрап. SimVPv2 — 12-шаговый горизонт, IAM4VP — 6-шаговый (native, §см. фикс
eval); сравниваются знаки и относительные Δ.

| лестница | A2: RMSE Δ% | A2: ACC Δпт | legacy: RMSE Δ% | legacy: ACC Δпт |
|---|---:|---:|---:|---:|
| SimVPv2 · France | **−1.23** | +0.03 | **+27.5** | −3.77 |
| SimVPv2 · N.Pacific | **−0.65** | −0.25 | **+49.0** | −6.59 |
| IAM4VP · France | **−1.99** | +0.06 | **+6.7** | −0.25 |
| IAM4VP · N.Pacific | **−3.55** | +0.18 | **+5.0** | −0.40 |

**Единогласный паттерн на всех 4 лестницах — H1 и H3 подтверждаются:**

1. **Корректная физика (A2) улучшает RMSE везде** (−0.7…−3.6 %), и **сильнее на
   авторегрессивном IAM4VP** (−2.0…−3.6 %), чем на MIMO SimVPv2 (−0.7…−1.2 %). Это
   ровно вывод exp21: физический приор работает тем лучше, чем сильнее авторегрессивная
   обратная связь носителя (у IAM4VP она встроена, у MIMO слабее).
2. **Легаси-физика (WeatherGFT с багами) вредит везде** (RMSE +5…+49 %), и **сильнее
   на MIMO SimVPv2** (+27…+49 %), чем на IAM4VP (+5…+7 %). Знак не переворачивается ни
   на одном регионе/носителе — единственный вывод exp16, воспроизведённый и здесь.
3. **Перенос на оба региона (H2/H1).** Знаки идентичны на Европе/Франции и Сев.
   Тихом океане; величины близки, без регион-специфичного разворота. Регион-зависимости
   (гипотеза о большем вкладе Q_θ во влажном/штормовом режиме) в интегральной RMSE не
   видно — но это по 4 из 6 лестниц и без разбора по уровням/лид-таймам.

Фигуры на лестницу: `metrics/results/<fam>_<region>/fig_ci_forest.png` (форест Δ к
no_physics), `fig_levels_*.png`, `fig_psd.png`, `heatmaps/<метрика>/<арм>.png`.
