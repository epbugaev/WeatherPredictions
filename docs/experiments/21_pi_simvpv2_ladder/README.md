...
**−9.3 % нельзя приписать физике.** В `chained` меняются сразу две вещи: корректор видит другой физпризнак `delta_phys` **и** более чистый `prev` (скорректированный кадр вместо сырого выхода бэкбона). Второе само по себе улучшает вход головы, безотносительно уравнений. Разделяет их только арм **S0c** (`chained` без физики), поданный после этой волны (джобы 4176155 / 4176156) — см. §4. Развилка:

- **S0c ≈ S0** → выигрыш S3c действительно физический: связка «включает» физику на MIMO;
- **S0c ≈ S3c** → физика ни при чём, работает сам механизм подачи `prev`, и главный вывод exp21 становится куда скромнее — но всё равно содержательным (он объясняет, почему MIMO-носителю физика «не даётся» без авторегрессивной связки).

**Это не плато.** 40 эпох — промежуточная точка обучения, а не сходимость. exp18 был отозван целиком именно за вывод, сделанный на промежуточной эпохе: на плато знаки перевернулись. Все выводы выше — **предварительные**, вердикт выносит long-волна (500 эпох, 8 арм, `val_every_n_epochs: 10`).

**Один сид (0).** Различия ≤ 1 п.п. (вся `batched`-лестница) не разрешены и могут быть шумом инициализации. Эффект связки (−9.3 %) на порядок больше этого масштаба.

### 8.5. Long-волна

*Идёт. Раздел заполняется по достижении общей эпохи.*
