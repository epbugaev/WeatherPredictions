...
### 8.6. Rollout-диагностика: 12 шагов нативным форвардом (2026-07-15)

Инференс-диагностика в формате exp16/exp20 (общее ядро `tools/rollout_ladder.py`), с чекпоинта **общей эпохи 500** для всех 8 армов, физединицы, lat-weighted RMSE по 69 каналам, полная вал-2004 (727 стартов). Инструменты: `rollout_eval.py` (кластер, CPU: 8 армов = 28 мин) → `results/rollout/*.npz` → `rollout_figures.py`. Отличия от авторегрессивных лестниц:

- **SimVPv2 — MIMO:** весь 12-шаговый горизонт выдаётся ОДНИМ форвардом, скользящего окна нет; **free-running и teacher-forced совпадают** (модель не потребляет промежуточных кадров), поэтому `fig_rollout_delta_steps` рисует одну панель, а не вырожденную пару (`RolloutSpec.single_mode`).
- **baseline единый — S0** (все 8 армов), в отличие от per-семейного baseline метрик: на ratio-оси и связка, и физика видны как лид-таймовая структура. Физику отдельно изолирует форест §8.5.

Фигуры: `fig_rollout_ratio_s0_t12.png` (RMSE ÷ S0 по шагам), `fig_rollout_delta_steps_t12.png` (Δ% к S0 по шагам), `fig_rollout_heatmap_t12.png` (Δ% по [уровень × шаг × арм]) + пер-армовые `fig_rollout_{heatmap,levels}_<арм>_t12.png`.

Средняя по 69 каналам Δ% RMSE к S0 (− = лучше S0):

| арм | шаг 1 | шаг 2 | шаг 6 | шаг 12 | ср. 1–12 |
| --- | --- | --- | --- | --- | --- |
| S1 · легаси-hybrid | +8.8 | +19.5 | +18.0 | +14.3 | +16.6 |
| S3a · A2 без Q_θ | +5.0 | +0.3 | +0.3 | +0.5 | +0.8 |
| S3 · A2 (exp13) | −3.8 | +0.5 | −0.2 | +0.0 | −0.4 |
| S4 · +exp14 | −4.1 | −0.3 | −0.6 | −0.3 | −0.8 |
| S5 · +exp15 | −3.9 | +0.3 | +0.0 | +0.3 | −0.2 |
| **S0c · chained без физики** | **+20.3** | **−29.5** | −13.2 | −6.0 | −10.8 |
| **S3c · A2 chained** | +6.7 | **−33.8** | **−17.6** | **−9.9** | **−15.9** |

Выводы (то, чего усреднённый по шагам форест §8.5 не показывает):

1. **Связка `chained` — это размен короткого лида на длинный.** S0c на шаге 1 **хуже** S0 на +20 % (z500 85.2 против 52.6 физ.ед — +62 %!), но с шага 2 уходит глубоко в минус (−29.5 %) и держит преимущество до шага 12 (−6 %). Модель, обученная free-running (val-loss доминируют поздние шаги), покупает точность длинного лида ценой первого шага. Форест, усредняя по 12 шагам, показывает лишь итог (−11 %) и прячет этот размен.
2. **Физика на chained смягчает штраф первого шага И углубляет длинный лид.** S3c против S0c: шаг 1 +6.7 % против +20.3 % (физика **срезает** штраф связки на коротком лиде втрое), а на шагах 2–12 идёт ещё ниже S0c (−33.8 против −29.5 на шаге 2, −9.9 против −6.0 на шаге 12). То есть корректная физика полезна на chained **на всех лидах** — и особенно чинит слабое место связки.
3. **Batched-армы — плоско у S0 на всех лидах**, кроме короткого выигрыша шага 1 (S3/S4/S5 −4 %), который к шагу 2 исчезает. Физика на нативной MIMO-подаче не двигает rollout — согласуется с форестом (∼0).
4. **Легаси S1 хуже S0 на всех лидах** (+9…+20 %), максимум штрафа — на шаге 2; знак не переворачивается ни на одном шаге. Heatmap подтверждает: r-налог легаси и z-деградация на всех уровнях.
