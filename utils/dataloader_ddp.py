    ...
    def __iter__(self):
        ...
        for next_input, next_target in self.loader:
            with torch.cuda.stream(stream):
                ...
                with autocast(enabled=self.fp16, dtype=self.amp_dtype):
...
# Добавить поддержку возобновления обучения
def save_dataloader_state(loader, path):
    if hasattr(loader.sampler, 'state_dict'):
        torch.save(loader.sampler.state_dict(), path)

def load_dataloader_state(loader, path):
    if hasattr(loader.sampler, 'load_state_dict'):
        loader.sampler.load_state_dict(torch.load(path))