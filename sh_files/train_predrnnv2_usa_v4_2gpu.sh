#!/bin/bash
#SBATCH --job-name=train-predrnnv2-usa-v4-2gpu
...
: "${ORIG_MEMMAP:?Set ORIG_MEMMAP to the packed USA memmap .dat for v4. Use predrnnv2_usa for the legacy WeatherBench files.}"
...
exec bash "${weatherpred__launcher}" predrnnv2_usa_v4 "$@"
