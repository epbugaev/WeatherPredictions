#!/bin/bash
#SBATCH --job-name=train-v4-memmap
...
# Model-agnostic v4 raw-memmap training launcher: the config is the only input,
# and the model comes from `model.type` inside it. Introduced for the exp21
# PI-SimVPv2 ladder; `train_pi_predrnnv2.sh` (exp20) has an identical body and
# should be retired in favour of this one once exp20 merges.
#
#   sbatch -J exp21-s3 sh_files/train_v4_memmap.sh configs/exp21/exp21_s3_a2_exp13_s0.yaml
set -euo pipefail
...
# several concurrent jobs on one node need 24 GB of /tmp EACH, and a node already
# hosting other waves runs out ("No space left on device" killed exp20 jobs
# 4175559/4175560). A 7-arm wave lands several jobs per node, so SKIP_STAGE=1 —
# reading the memmap straight off the shared filesystem — is the safe default for
# a whole ladder; unset it for a single job that wants the fast path.
...
weatherpred__config="${1:?Pass the config path, e.g. configs/exp21/exp21_s0_no_physics_s0.yaml}"
shift
