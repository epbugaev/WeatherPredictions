#!/bin/bash
#SBATCH --job-name=phys-fixA-alphaOmega
...
# =============================================================================
# Fix A: заменить Q = -L·z_z·w → Q = R_d·T·ω/(c_p·p) (adiabatic heating).
# Также R → R_d в гидростатике.
# CPU-only.
# =============================================================================
set -euo pipefail
...
echo "[phys-fixA] JobID=${SLURM_JOB_ID:-local} host=$(hostname) git_sha=$(git rev-parse --short HEAD 2>/dev/null)"
