#!/bin/bash
#SBATCH --job-name=train-iam4vp-usa-v4-2gpu
...
# Stages the packed memmap to node-local /tmp so dataloader reads are NVMe, not Lustre.
set -euo pipefail
...
exec bash "${weatherpred__launcher}" iam4vp_usa_v4 "$@"
