#!/bin/bash
#SBATCH --job-name=train-weathergft-single-usa-v4-2gpu
...
: "${ORIG_MEMMAP:?Set ORIG_MEMMAP to the packed USA memmap .dat for v4. Use weathergft_single_usa for the legacy WeatherBench files.}"
...
exec bash "${weatherpred__launcher}" weathergft_single_usa_v4 "$@"
