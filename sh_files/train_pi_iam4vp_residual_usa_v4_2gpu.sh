#!/bin/bash
#SBATCH --job-name=train-pi-iam4vp-residual-usa-v4-2gpu
...
# v4 raw-memmap training for PI-IAM4VP residual-corrector experiments.
# Pass an optional config stem as the first positional argument, e.g.
#   sh_files/train_pi_iam4vp_residual_usa_v4_2gpu.sh pi_iam4vp_residual_no_physics_usa_v4
set -euo pipefail
...
weatherpred__config="${1:-pi_iam4vp_residual_usa_v4}"
if [[ $# -gt 0 ]]; then
  shift
fi
