#!/bin/bash
#SBATCH --job-name=train-pi-iam4vp-usa-v4-2gpu
...
: "${ORIG_MEMMAP:?Set ORIG_MEMMAP to the packed USA memmap .dat for v4. Use pi_iam4vp_usa for the legacy WeatherBench files.}"
...
exec bash "${weatherpred__launcher}" pi_iam4vp_usa_v4 "$@"
