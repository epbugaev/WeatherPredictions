#!/bin/bash
#SBATCH --job-name=PredFormer-USA-2gpu-v5
...
# =============================================================================
# Production training of PredFormer-USA-v5 on 2 GPUs.
# Same recipe as v4 (bf16 + fused AdamW + warmup + grad_clip +
# val_every_n=3 + async ckpt) but uses the v5 dataset and the
# CudaPrefetcher pipeline (background-thread pinning + side-stream H2D)
# to overlap data transfer with compute.
# =============================================================================
set -euo pipefail
...
mkdir -p "${STAGE_DIR}"
echo "[train-2gpu-v5] staging memmap ${ORIG_MEMMAP} -> ${STAGE_DIR}/"
...
echo "[train-2gpu-v5] staged in ${weatherpred__stage_secs}s: $(du -sh "${STAGE_DIR}" | cut -f1)"
...
exec bash "${weatherpred__launcher}" predformer_usa_v5 "$@"
