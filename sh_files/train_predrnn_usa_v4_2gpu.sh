#!/bin/bash
#SBATCH --job-name=train-predrnn-usa-v4-2gpu
...
: "${ORIG_MEMMAP:?Set ORIG_MEMMAP to the packed USA memmap .dat for v4. Use predrnn_usa for the legacy WeatherBench files.}"
...
exec bash "${weatherpred__launcher}" predrnn_usa_v4 "$@"
