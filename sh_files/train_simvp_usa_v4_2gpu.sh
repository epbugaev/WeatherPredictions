#!/bin/bash
#SBATCH --job-name=train-simvp-usa-v4-2gpu
...
: "${ORIG_MEMMAP:?Set ORIG_MEMMAP to the packed USA memmap .dat for v4. Use simvp_usa for the legacy WeatherBench files.}"
...
exec bash "${weatherpred__launcher}" simvp_usa_v4 "$@"
