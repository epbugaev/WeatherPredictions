...
# =============================================================================
# Production training of PredFormer-USA-v4 on 2 GPUs.
# Same staging trick as ``train_PredFormer_USA_2gpu.sh``; only difference is
# the config — ``predformer_usa_v4.yaml`` uses the v4 dataset (raw memmap)
# with normalisation performed by the trainer via WeatherNormalize.
# =============================================================================
set -euo pipefail
...
echo "[train-2gpu-v4] staged in ${weatherpred__stage_secs}s: $(du -sh "${STAGE_DIR}" | cut -f1)"
...
exec bash "${weatherpred__launcher}" predformer_usa_v4 "$@"
