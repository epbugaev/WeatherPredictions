...
def dump(name: str, t: torch.Tensor | float) -> None:
    ...
    if finite.numel() == 0:
        print(f"  {name:<35s}  shape={tuple(arr.shape)}  ALL_NONFINITE  NaN={has_nan} Inf={has_inf}")
        return
    print(
...
def main() -> None:
    ...
    print(f"=== Diagnostic first-substep dump ===")
    ...
    u_x = ops.d_x(u); dump("u_x = ∂u/∂x (1/s)", u_x)
    u_y = ops.d_y(u); dump("u_y = ∂u/∂y (1/s)", u_y)
    u_z = ops.d_z(u); dump("u_z = ∂u/∂p (m/(s·hPa))", u_z)
    v_x = ops.d_x(v); dump("v_x", v_x)
    v_y = ops.d_y(v); dump("v_y", v_y)
    v_z = ops.d_z(v); dump("v_z", v_z)
    t_x = ops.d_x(t); dump("t_x (K/m)", t_x)
    t_y = ops.d_y(t); dump("t_y", t_y)
    t_z = ops.d_z(t); dump("t_z (K/hPa)", t_z)
    z_x = ops.d_x(z); dump("z_x (m²/s²/m)", z_x)
    z_y = ops.d_y(z); dump("z_y", z_y)
    z_z = ops.d_z(z); dump("z_z (m²/s²/hPa)", z_z)
    q_x = ops.d_x(q); dump("q_x", q_x)
    q_y = ops.d_y(q); dump("q_y", q_y)
    q_z = ops.d_z(q); dump("q_z", q_z)

    print(f"\n[STAGE 4] diagnostic w = integral_z(-(u_x + v_y)):")
    w_z = -(u_x + v_y); dump("w_z = -(u_x+v_y) (1/s)", w_z)
    w = ops.integral_z(w_z); dump("w = integral_z(w_z)  (hPa/s?)", w)
    print("  NB: w units here are hPa·(1/s) = hPa/s, не m/s! "
          "Это omega (pressure velocity), а не вертикальная скорость в физ.метрах.")

    print(f"\n[STAGE 5] heating Q = -L · z_z · w  (the suspect):")
    Q = -L * z_z * w; dump("Q = -L·z_z·w", Q)
    ...
    dt = BLOCK_DT
    u_new = u + dt * u_t; dump("u_new", u_new)
    v_new = v + dt * v_t; dump("v_new", v_new)
    t_new = t + dt * t_t; dump("t_new", t_new)
    z_new = z + dt * z_t; dump("z_new", z_new)
    q_new = q + dt * q_t; dump("q_new", q_new)
    ...
    print(f"    |u|  ≈ 50 m/s          → realistic Δu/hour ≈ 2 m/s, per 300s ≈ 0.2 m/s")
    print(f"    |t|  ≈ 250 K           → realistic ΔT/hour ≈ 0.5 K,  per 300s ≈ 0.04 K")
    print(f"    |z|  ≈ 5e4 m²/s²       → realistic Δz/hour ≈ 1e2 m²/s², per 300s ≈ 8 m²/s²")
    print(f"    |q|  ≈ 1e-2 kg/kg      → realistic Δq/hour ≈ 1e-4,    per 300s ≈ 1e-5")
    ...
    for name, x in (("u_new", u_new), ("v_new", v_new), ("t_new", t_new), ("z_new", z_new), ("q_new", q_new)):
