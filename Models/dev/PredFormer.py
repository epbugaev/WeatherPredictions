...
from Models.PredFormerUtils import TriangularCausalMask
...
class GatedTransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None, WindowAttention=False):
        ...
        x = self.norm2(x)
        x = self.dropout(
            self.activation(self.conv1(x.transpose(-1, 1))) * self.conv2(x.transpose(-1, 1))
        )
...
class FullAttentionLayer(nn.Module):
    def __init__(self, attention, d_model, d_ff=None, dropout=0.1):
        ...
        self.gtb = GatedTransformerBlock(
            attention=attention, d_model=d_model, d_ff=d_ff, dropout=dropout
        )
...
class BinaryTSLayer(nn.Module):
    def __init__(self, attention_t, attention_s, d_model, d_ff=None, dropout=0.1):
        ...
        self.gtb_t = GatedTransformerBlock(
            attention=attention_t, d_model=d_model, d_ff=d_ff, dropout=dropout
        )

        self.gtb_s = GatedTransformerBlock(
            attention=attention_s, d_model=d_model, d_ff=d_ff, dropout=dropout
        )
    ...
    def forward(self, x, attn_mask=None):
        ...
        x, attn = self.gtb_s(
            x, attn_mask=attn_mask, WindowAttention=True
        )  # No attn_mask for Space, applying WindowAttention
