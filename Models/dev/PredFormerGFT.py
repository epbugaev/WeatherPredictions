...
class HybridBlock(nn.Module):
    ...
    def forward(self, x, zquvtw=None, x_history=None):
        if self.use_pde:
            ...
            # Adaptive Router
            PredFormerGFT.layer_weights[f"{self.__class__.__name__}_{PredFormerGFT.gft_name}"] = (
        ...
        for i_layer in range(len(body_layers)):
            if use_checkpoint:
                layer = checkpoint_wrapper(
                    StageModule(
                        in_channels=body_dim_list[i_layer],
                        hidden_dimension=body_dim_list[i_layer + 1],
                        layers=body_layers[i_layer],
                        physics_part_coef=physics_part_coef,
                        scaling_factors=body_scaling_factors[i_layer],
                        num_heads=body_heads[i_layer],
                        head_dim=head_dim,
                        window_size=window_size,
                        relative_pos_embedding=relative_pos_embedding,
                        use_pde=True,
