    ...
    def get_t_t(self, u, v, w, t, q, q_s=None):
        ...
        if q_s is None:
            ...
            e_s = (
                ...
                * torch.exp(
                    self.scale_tensor(
                        17.67 * t_celsius / self.avoid_inf(t_celsius + 243.5), -3.47, 3.01
                    )
                )
                * 100
            )
...
def analyze_physics_components(model, input_data):
    ...
    def hook_t_dt(module, input, output):
        ...
        adiabatic_t = (
            torch.abs((-module.L * module.z_z * w - module.z_z * w) / module.c_p).mean().item()
        )
        ...
        e_s = (
            ...
            * torch.exp(
                module.scale_tensor(
                    17.67 * t_celsius / module.avoid_inf(t_celsius + 243.5), -3.47, 3.01
                )
            )
            * 100
        )
        ...
        contributions["turbulent_mixing"][-1] = (
            contributions["turbulent_mixing"][-1] + mixing_t
        ) / 2
    ...
    for module in model.modules():
        if isinstance(module, PDE_kernel):
            handles.append(
                module.get_uv_dt.__func__.__get__(module, PDE_kernel).register_forward_hook(
                    hook_uv_dt
                )
            )
            handles.append(
                module.get_t_t.__func__.__get__(module, PDE_kernel).register_forward_hook(hook_t_dt)
            )
