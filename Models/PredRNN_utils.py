...
class SpatioTemporalLSTMCellv2(nn.Module):
    ...
    Identical recurrence to :class:`SpatioTemporalLSTMCell` but additionally
    returns the per-step memory increments ``delta_c`` / ``delta_m`` used by
    PredRNN-V2's decoupling loss. LayerNorm is sized for a rectangular
    ``(height, width)`` latent grid (the upstream reference assumes square
    inputs; this repo runs the 32x64 USA crop).
