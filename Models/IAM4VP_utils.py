"""Строительные блоки IAM4VP: circular-padding conv-стек и ConvNeXt-блоки.

Содержит conv-примитивы с циклическим паддингом по долготе
(``CircularConv2d``/``CircularConvSC``), time-conditioned ConvNeXt-блоки
(``ConvNeXt_block``/``ConvNeXt_bottle``) и LKA-attention, из которых
собирается encoder/predictor/decoder в ``Models/IAM4VP.py``.
"""
